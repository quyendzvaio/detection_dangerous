...
class EvidenceLifecycleResponse(BaseModel):
    ...
    # Short-lived signed download URLs, generated by the cloud backend after a
    # successful upload. The worker forwards them to the customer host via
    # MQTT so the local backend can pull evidence into its own storage.
