...
def tenant_id(db_session) -> int:
    tenant = Tenant(tenant_key="test", name="test tenant")
...
def test_camera_config_revision_and_runtime_registration_ownership(
...
def test_partial_model_update_does_not_overwrite_other_toggles(db_session, tenant_id):
    camera = camera_service.create_camera(
        db_session,
        tenant_id,
        CameraCreate(
...
def test_zone_delete_is_soft_and_changes_camera_revision(db_session, tenant_id):
    camera = camera_service.create_camera(
        db_session,
        tenant_id,
    ...
    zone_service.delete_zone(db_session, tenant_id, zone.id)

    persisted = db_session.get(Zone, zone.id)
    assert persisted is not None
    assert persisted.deleted_at is not None
    assert persisted.is_active is False
    assert zone_service.get_zone_by_id(db_session, tenant_id, zone.id) is None
    assert zone_service.get_zones(db_session, tenant_id, camera_id=camera.id) == []
    assert camera.config_revision == 3


def test_camera_status_updates_last_seen_and_config_state(db_session, tenant_id):
    camera = camera_service.create_camera(
        db_session,
        tenant_id,
...
def test_runtime_config_includes_active_zones_and_acknowledges_revision(
    db_session, tenant_id
):
    camera = camera_service.create_camera(
        db_session,
    ...
    runtime_config = camera_service.get_runtime_config(db_session, tenant_id, camera.id)
...
def test_runtime_config_rejects_stale_ack(db_session, tenant_id):
    camera = camera_service.create_camera(
        db_session,
        tenant_id,
    ...
    camera_service.update_camera(
        db_session, tenant_id, camera.id, CameraUpdate(ppe_enabled=True)
    )
